...
class ChartNetworkingMock: NetworkingServiceProtocol {
    let bundle = Bundle(for: ChartNetworkingMock.self)

    func doRequest<T>(targetType: TargetType, completion: @escaping (Result<T, Error>) -> Void) where T: Codable {
        if let path = bundle.path(forResource: "ChartData", ofType: "json"),
