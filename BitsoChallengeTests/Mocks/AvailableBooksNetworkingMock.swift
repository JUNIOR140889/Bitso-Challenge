...
class AvailableBooksNetworkingMock: NetworkingServiceProtocol {
    let bundle = Bundle(for: AvailableBooksNetworkingMock.self)

    func doRequest<T>(targetType: TargetType, completion: @escaping (Result<T, Error>) -> Void) where T: Codable {
        if let path = bundle.path(forResource: "AvailableBooks", ofType: "json"),
